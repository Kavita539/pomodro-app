...
import { useTimer, useTask } from "../../context";
import { useParams } from "react-router-dom";

const Timer = ({ setModalSetting }) => {
	const {
		timeState: {
			workTimeMinutes,
			breakTimeMinutes
	...
	const [secondsLeft, setSecondsLeft] = useState(Number(workTimeMinutes) * 60);
	...
	useEffect(() => {
		...
		const timerId = setInterval(() => {
			...
			if (secondsRef.current === 0) {
				...
				const nextSeconds =
					(nextMode === "work" ? Number(workTimeMinutes) : Number(breakTimeMinutes)) *
					60;
				setMode(nextMode);
			...
			<div className="flex h-4/5 flex-col gap-2">
				...
				<div className="flex justify-center items-center flex-col ">
					<div className="flex justify-center items-center gap-4">
						<RefreshIcon
							className="w-10 h-10  border-pink-300 text-white  cursor-pointer border-2  dark:border-blue-600 rounded-full p-1 "
							onClick={() => {
								secondsRef.current =
									mode === "work"
										? Number(workTimeMinutes) * 60
										: Number(breakTimeMinutes) * 60;
								setSecondsLeft(secondsRef.current);
							}}
						/>
						<span>
							{isPaused ? (
								<PlayIcon
									className="w-12 h-12 text-pink-400 cursor-pointer "
									onClick={() => {
										setIsPaused(false);
								...
								<PauseIcon
									className="w-12 h-12 cursor-pointer text-pink-400"
									onClick={() => {
										setIsPaused(true);
					...
					<div
						...
						onClick={() => setModalSetting((modalSetting) => !modalSetting)}
					>
